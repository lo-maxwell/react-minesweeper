...
function massageQueryParams(query_params: ParsedUrlQuery) {
	...
	let columns = Array.isArray(query_params.columns) ? query_params.columns : (query_params.columns ? [query_params.columns] : []);
	let where: Record<string, string> = {};
	if (query_params.where) {
		const whereClauses = Array.isArray(query_params.where) ? query_params.where : [query_params.where];
...
//Exposes api at /api/leaderboard/viewEntriess
export async function GET(request: Request) { 
	const parsedUrl = URL.parse(request.url, true);
	const pathname = parsedUrl.pathname;
	const query = parsedUrl.query;
	console.log(query);
	const {columns, where, orderBy, limit} = massageQueryParams(query); //TODO: Fix query
